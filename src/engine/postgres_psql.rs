...
impl Engine for PSQL {
    async fn execute_with_writer(
        ...
        // 4. Drain stderr in background (prevents deadlock). It's always
        //    captured to a buffer so it's available for the ExecutionFailed
        //    error case, and additionally forwarded to stderr_writer below.
        ...
        if let Some(mut sink) = stderr_writer {
            use tokio::io::AsyncWriteExt;
            let _ = sink.write_all(&stderr_bytes).await;
        }
...
impl PSQL {
    ...
    async fn execute_sql(
        ...
        self.execute_with_writer(
            ...
            None, // Not capturing stderr for internal queries
        )
        .await
    ...
    async fn record_migration(
        ...
        self.execute_with_writer(
            ...
            None,
            None, // Not capturing stderr for recording migrations
        )
        .await
        .map_err(|e| match e {
    ...
    async fn apply_and_record_migration_v1(
        ...
        let migration_result = self
            .execute_with_writer(
                ...
                None,
                None, // Not capturing stderr for migration apply
            )
            .await;
