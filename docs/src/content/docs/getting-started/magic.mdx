...
The error tells us that we need to either pin the migration, or use `--no-pin`. Run like so:
...
But if we edit the target in `spawn.toml` to specify that it is a prod environment, like so:
