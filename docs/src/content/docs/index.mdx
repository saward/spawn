...
Spawn solves these painpoints through two mechanisms:

1. Separate components (views, functions, triggers, reusable snippets) from the migrations.
1. Content Addressable Storage (CAS), much like git, for components. Pin migrations to specific versions of components.
1. Dynamic SQL generation via templating.
...
generate script path: spawn/migrations/20260124064957-add_fee/up.sql
BEGIN;
...
```bash
# Pinned version of old migration uses old version of the function:
...
# New migration uses the new version of the function:
% spawn migration build 20260124073534-update-add-func --pinned
generate script path: spawn/migrations/20260124073534-update-add-func/up.sql
BEGIN;
...
INSERT INTO fee (name, amount) VALUES ('base', 0.12);
INSERT INTO fee (name, amount) VALUES ('high-test', 0.98); -- HERE
