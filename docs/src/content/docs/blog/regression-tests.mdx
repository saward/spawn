...
Let's make a real test. First order of business, we want to create a copy of the database so that we can rerun the test multiple times without making permanent changes to our database. We _could_ wrap the test in a transaction and roll it back (Spawn works fine this way), but some operations can’t run inside a transaction block (like creating databases), and sometimes you want to test behavior across commits. Using `WITH TEMPLATE` gives you a clean slate every time without relying on rollback.
...
![Diff image](/src/assets/blog/regression-tests/example_diff.png)
...
As your tests grow, you may want to explore using `json` sources to loop over multiple test cases to create data driven tests. Hopefully this shows you a little idea of the possibilities for testing your PostgreSQL database using Spawn. There's a short introduction covering both Spawn's migration and testing capabilities in [The Magic of Spawn](/getting-started/magic/), if you'd like to learn more about its migration features.
